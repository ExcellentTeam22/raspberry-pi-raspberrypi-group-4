...
int main() {
    Mat frame, oMatCrop;//declaring a matrix to video frame in it//
    ...
    face_cascade.load("C:/opencv/sources/data/haarcascades/haarcascade_frontalface_default.xml");//loading the cascade classifier//
    ...
    while (true) {
        ...
        for (int i = 0; i < faces.size(); i++) { //for locating the face
            ...
            for (size_t j = 0; j < eyes.size(); j++) { //for locating eyes//
                ...
                oMatCrop = frame(cv::Rect(center.x - (eyes[j].width / 2), center.y - (eyes[j].height / 2), eyes[j].width, eyes[j].height));
                cv::Mat image = frame;
                for (int i = 0; i < oMatCrop.rows; i++)
                {
                    for (int j = 0; j < oMatCrop.cols; j++)
                    {
                        Vec3b bgrPixel = oMatCrop.at<Vec3b>(i, j);
                        std::cout << oMatCrop.at<Vec3b>(i, j);
